...
if(isset($_POST['delete'])){
   ...
   $verify_delete = $conn->prepare("SELECT * FROM `property_1` WHERE id = ?");
   $verify_delete->execute([$delete_id]);

   if($verify_delete->rowCount() > 0){
      $select_images = $conn->prepare("SELECT * FROM `property_1` WHERE id = ?");
      ...
      $delete_listings = $conn->prepare("DELETE FROM `property_1` WHERE id = ?");
...
<section class="listings">
   ...
   <?php
      if(isset($_POST['search_box']) OR isset($_POST['search_btn'])){
         ...
         $select_listings = $conn->prepare("SELECT * FROM `property_1` WHERE property_name LIKE '%{$search_box}%' OR address LIKE '%{$search_box}%' ORDER BY date DESC");
         $select_listings->execute();
      }else{
         $select_listings = $conn->prepare("SELECT * FROM `property_1` ORDER BY date DESC");
         $select_listings->execute();
      } 
      $total_images = 0;
   ...
   <div class="box">
      ...
      <p class="price"><i class="fas fa-naira-sign"></i><?= $fetch_listing['price']; ?></p>
      ...
      <form action="" method="POST">
         ...
         <div class="flex-btn">
            ...
            <input type="submit" value="delete" onclick="return confirm('delete this listing?');" name="delete" class="delete-btn">
        </div>
