...
$select_account = $conn->prepare("SELECT * FROM `agents` WHERE id = ? LIMIT 1");
$select_account->execute([$agent_id]);
...
if(isset($_POST['submit'])){
    ...
    if(!empty($email)){
        $verify_emil = $conn->prepare("SELECT email FROM `agents` WHERE email = ?");
    ...
    if($empty_pass != $old_pass){
        if($old_pass != $prev_pass){
            ...
            if($new_pass != $empty_pass){
                $update_pass = $conn->prepare("UPDATE `agents` SET password = ?
                 WHERE id = ?");
