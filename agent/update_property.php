...
if(isset($_POST['update'])){
   ...
   if(!empty($image_01)){
      if($image_01_size > 2000000){
         ...
         $update_image_01 = $conn->prepare("UPDATE `property_1` SET image_01 = ? WHERE id = ?");
   ...
   if(!empty($image_02)){
      if($image_02_size > 2000000){
         ...
         $update_image_02 = $conn->prepare("UPDATE `property_1` SET image_02 = ? WHERE id = ?");
   ...
   if(!empty($image_03)){
      if($image_03_size > 2000000){
         ...
         $update_image_03 = $conn->prepare("UPDATE `property_1` SET image_03 = ? WHERE id = ?");
   ...
   if(!empty($image_04)){
      if($image_04_size > 2000000){
         ...
         $update_image_04 = $conn->prepare("UPDATE `property_1` SET image_04 = ? WHERE id = ?");
   ...
   if(!empty($image_05)){
      if($image_05_size > 2000000){
         ...
         $update_image_05 = $conn->prepare("UPDATE `property_1` SET image_05 = ? WHERE id = ?");
   ...
   $update_listing = $conn->prepare("UPDATE `property_1` SET property_name = ?, address = ?, price = ?, type = ?, offer = ?, status = ?, furnished = ?, bhk = ?, deposite = ?, bedroom = ?,
    bathroom = ?, carpet = ?, age = ?, total_floors = ?, room_floor = ?, loan = ?, lift = ?,
    security_guard = ?, play_ground = ?, garden = ?, water_supply = ?, power_backup = ?, parking_area = ?, gym = ?, shopping_mall = ?, hospital = ?, school = ?, market_area = ?, description = ? WHERE id = ?");   
   $update_listing->execute([$property_name, $address, $price, $type, $offer, $status, $furnished,
    $bhk, $deposite, $bedroom, $bathroom, $carpet, $age, $total_floors, $room_floor, $loan, $lift, $security_guard,
     $play_ground, $garden, $water_supply, $power_backup, $parking_area, $gym, $shopping_mall, $hospital, $school, $market_area, $description, $update_id]);
...
<section class="property-form">
   ...
   <?php
      $select_properties = $conn->prepare("SELECT * FROM `property_1` WHERE id = ? ORDER BY date DESC LIMIT 1");
   ...
   <form action="" method="POST" enctype="multipart/form-data">
      ...
      <div class="flex"> 
         <div class="box">
            ...
            <input type="submit" value="delete image 02" 
            name="delete_image_02" class="delete-btn" onclick="return confirm
            ('delete image 02');">
            <?php } ?>
            <p>update image 02</p>
         ...
         <div class="box">
            ...
            <input type="submit" value="delete image 03" name="delete_image_03" class="delete-btn" onclick="return confirm('delete image 03');">
            <?php } ?>
            <p>update image 03</p>
         ...
         <div class="box">
            ...
            <input type="submit" value="delete image 04" name="delete_image_04" class="delete-btn" onclick="return confirm('delete image 04');">
            <?php } ?>
            <p>update image 04</p>
         ...
         <div class="box">
            ...
            <input type="submit" value="delete image 05" name="delete_image_05" class="delete-btn" onclick="return confirm('delete image 05');">
            <?php } ?>
            <p>update image 05</p>
