...
if(isset($_POST['delete'])){
   ...
   if($verify_delete->rowCount() > 0){
      $select_images = $conn->prepare("SELECT * FROM `property_1` WHERE agent_id = ?");
      ...
      $delete_listings = $conn->prepare("DELETE FROM `property_1` WHERE agent_id = ?");
...
<section class="grid">
   ...
   <h1 class="heading">Aents</h1>

   <form action="" method="POST" class="search-form">
      <input type="text" name="search_box" placeholder="search Agents..." maxlength="100" required>
      <button type="submit" class="fas fa-search" name="search_btn"></button>
   </form>
   <div class="box" style="order: -1; width: 30%; display: flex; padding:1.7rem;" >
      <a href="register_agent.php" class="btn">register new Agent</a>
   </div>
   ...
   <?php
      ...
      if($select_agents->rowCount() > 0){
         while($fetch_agents = $select_agents->fetch(PDO::FETCH_ASSOC)){
            ...
            $count_property = $conn->prepare("SELECT * FROM `property_1` WHERE agent_id = ?");
